...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        //GETで現在登録されているのを取得
        const data = yield rest.get(Routes.applicationCommands(config.client, config.server));
        console.log("---コマンド一覧---");
        ...
        switch (mode) {
            case '登録(更新)': {
                ...
                if (prompt) {
                    ...
                    yield rest.put(Routes.applicationCommands(config.client), { body: commands })
            ...
            case '削除': {
                const selected = yield new MultiSelect({
                    name: 'value',
                ...
                for (const selectedElement of selected) {
                    ...
                    yield rest.delete(Routes.applicationCommand(config.client, selectedElement[1]))
