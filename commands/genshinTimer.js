...
module.exports = [
    {
        data: new SlashCommandBuilder().setName("genshin-timer").setDescription("About genshin-timer discord bot"),
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .addStringOption((option) => option.setName("現在").setDescription("現在の天然樹脂の数を入力します").setRequired(true))
            .addStringOption((option) => option.setName("通知量").setDescription("通知してほしい樹脂の数を入力します").setRequired(true))
            .addStringOption((option) =>
                option.setName("次の回復").setDescription("次の回復までの時間を分単位で入力します").setRequired(false)
            ),
        async execute(interaction) {
            let second;
            if (interaction.options.getString("次の回復") === null) {
                ...
                second =
                    (interaction.options.getString("通知量") - interaction.options.getString("現在")) * 8 -
                    (8 - interaction.options.getString("次の回復"));
            }
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .addStringOption((option) =>
                option.setName("スコア").setDescription("聖遺物のスコア(サブOPの率✕2+ダメージ+攻撃%)").setRequired(true)
            ),
        async execute(interaction) {
