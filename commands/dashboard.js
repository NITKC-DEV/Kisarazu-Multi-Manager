...
module.exports = [
    {
        data: new SlashCommandBuilder().setName("dashboard").setDescription("ダッシュボードを表示します"),

        async execute(interaction) {
            if (interaction.guild === undefined || interaction.guild === null) {
                await interaction.reply({
                    content: "サーバー情報が取得できませんでした。DMで実行している などの原因が考えられます。",
                    ephemeral: true,
                });
            } else {
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .addIntegerOption((option) => option.setName("四半期").setDescription("何番目のテストか入力(1~4)").setRequired(true)),
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .setDefaultMemberPermissions(1 << 3)
            .addStringOption((option) => option.setName("ダッシュボードid").setDescription("メッセージIDを入力").setRequired(true))
            .addStringOption((option) => option.setName("チャンネルid").setDescription("チャンネルIDを入力").setRequired(true))
