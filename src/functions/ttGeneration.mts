...
export const generation = async function func(grade: any, department: any, day: any, change = true): Promise<number | EmbedBuilder> {
    ...
    if (change) {
        ...
        data = await db.find("main", "timetableData", {
            ...
            day: String((date.getMonth() + 1) * 100 + date.getDate()),
        });
        if (data.length === 0) {
...
export const setNewTimetableData = async function func(interaction: any) {
    ...
    const embed = new EmbedBuilder()
        .setColor(0x00a0ea)
        .setTitle(
            `授業変更・定期テスト登録 - ${departmentData[parseFloat(department) - 1].name}${grade}年 ${Math.floor(
                // @ts-expect-error TS(2362): The left-hand side of an arithmetic operation must... Remove this comment to see the full error message
                date / 10000,
                // @ts-expect-error TS(2362): The left-hand side of an arithmetic operation must... Remove this comment to see the full error message
            )}月${Math.floor((date % 10000) / 100)}日`,
        )
        .setAuthor({
            name: "木更津高専統合管理BOT",
            iconURL: "https://media.discordapp.net/attachments/1004598980929404960/1039920326903087104/nitkc22io-1.png",
    ...
    try {
        ...
        channel.messages.fetch(interaction.message.id).then((message: any) => {
