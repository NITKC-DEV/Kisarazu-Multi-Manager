...
export default [
    ...
    },
    {
        ...
        async execute(interaction: any) {
            await interaction.reply({
                content:
                    "DMに管理者向けメニューを送信しました。受信できていない場合、以下に該当していないかどうかご確認ください。\n・このサーバー上の他のメンバーからのDMをOFFにしている\n・フレンドからのDMのみを許可している\n・このBOTをブロックしている",
                ephemeral: true,
            });
    ...
    {
        data: new SlashCommandBuilder().setName("about").setDescription("このBOTの概要を表示します"),
        async execute(interaction: any) {
    ...
    {
        data: new SlashCommandBuilder().setName("ping").setDescription("このBOTのpingを測定します"),
        async execute(interaction: any) {
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .setDefaultMemberPermissions(1 << 3)
            .setDMPermission(false)
            .addBooleanOption((option: any) => option.setName("option").setDescription("True or False").setRequired(true)),
        async execute(interaction: any) {
            ...
            if (flag === 1) {
                ...
                await reply
                    .awaitReactions({ filter: (reaction: any) => reaction.emoji.name === "⭕" || reaction.emoji.name === "❌", max: 1 })
                    .then(() => {
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .addStringOption((option: any) =>
                option.setName("メッセージ").setDescription("送りたいメッセージを入れます").setRequired(false),
            )
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .setDMPermission(false)
            .addIntegerOption((option: any) => option.setName("年").setDescription("生まれた年をいれます").setRequired(true))
            .addIntegerOption((option: any) => option.setName("月").setDescription("生まれた月をいれます").setRequired(true))
            .addIntegerOption((option: any) => option.setName("日").setDescription("生まれた日をいれます").setRequired(true)),
        ...
        async execute(interaction: any) {
            ...
            if (
                interaction.options.getInteger("月") > 0 &&
                interaction.options.getInteger("月") < 13 &&
                interaction.options.getInteger("日") > 0 &&
                interaction.options.getInteger("日") < 32 &&
                interaction.options.getInteger("年") ** 2 >= 0
            ) {
                if (data.length > 0) {
                    await db.update(
                        "main",
                ...
                await interaction.editReply({
                    content: `このサーバーで誕生日を${interaction.options.getInteger("月")}月${interaction.options.getInteger(
                        "日",
    ...
    },
    {
        ...
        async execute(interaction: any) {
            ...
            if (data.length > 0) {
                ...
                await interaction.editReply({
                    content: "このサーバーでの通知を解除しました。\n他のサーバーでも通知を止めたい場合、そのサーバーで実行してください。",
                });
            } else {
                await interaction.editReply({
                    content: "このサーバーではあなたの誕生日が設定されていません。\n通知を止めたいサーバーで実行してください。",
                });
            }
        },
    },
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .addBooleanOption((option: any) =>
                option.setName("定期送信").setDescription("定期実行のオンオフを指定します").setRequired(true),
            )
        ...
        async execute(interaction: any) {
            ...
            if (!interaction.options.getBoolean("定期送信")) {
                ...
                await guildData.updateOrInsert(interaction.guildId, {
                    weather: interaction.options.getBoolean("定期送信"),
