...
export default [
        ...
        async execute(interaction: any) {
            ...
            if (isNaN(grade)) {
                ...
                if (!interaction.guild) {
                    ...
                    await interaction.editReply(
                        "あなたが学科ロールを付けていないか、このサーバーに学科ロールが登録されていないため、学科オプションを省略できません。\nサーバーでロールを付与してもらうか、管理者に伝えて/guilddataで学科ロールを登録してもらってください。",
                    );
                }
            } else {
                const embed = await timetable.generation(
                    grade,
                    department,
                    String(dayOfWeek),
                    interaction.options.getBoolean("授業変更") ?? true,
                );
                if (embed === 0) {
    ...
    },
    {
        data: new SlashCommandBuilder()
            ...
            .addBooleanOption((option: any) => option.setName("options").setDescription("定期実行の可否を指定します").setRequired(true)),
    ...
    },
    {
        ...
        async execute(interaction: any) {
            ...
            if (defaultData[0] === undefined) {
                ...
                if (defaultData[0] === undefined) {
                    ...
                    if (defaultData[0] === undefined) {
                        ...
                        date = new Date(
                            date.getFullYear(),
                            interaction.options.getInteger("変更日") / 100 - 1,
                            parseFloat(interaction.options.getInteger("変更日")) % 100,
                        );
            ...
            const subject = await db.find("main", "syllabusData", {
                subject_id: `${interaction.options.getString("学年")}${interaction.options.getString("学科")}`,
            });
            ...
            if (interaction.options.getString("モード") === "1") {
                for (let i = 0; i < 4; i++) {
                    select[i] = new StringSelectMenuBuilder()
                        .setCustomId(
                            `${interaction.options.getString("学年")}${interaction.options.getString(
                                "学科",
                            )}${day}${interaction.options.getInteger("変更日")}changeTimetableSelectMenu${interaction.options.getString(
                                "モード",
                            )}${i}`,
                        )
            ...
            const embed = new EmbedBuilder()
                ...
                .setTitle(
                    `授業変更・定期テスト登録 - ${
                        departmentData[parseFloat(interaction.options.getString("学科")) - 1].name
                    }${interaction.options.getString("学年")}年 ${Math.floor(interaction.options.getInteger("変更日") / 100)}月${Math.floor(
                        interaction.options.getInteger("変更日") % 100,
                    )}日`,
                )
                .setAuthor({
                    name: "木更津高専統合管理BOT",
            ...
            const button = new ButtonBuilder({
                custom_id: `${interaction.options.getString("学年")}${interaction.options.getString(
                    "学科",
                )}${interaction.options.getInteger("変更日")}changeTimetableButton${interaction.options.getString("モード")}`,
                style: 1,
                label: "登録！",
            });
            if (interaction.options.getString("モード") === "1") {
    ...
    },
    {
        ...
        async execute(interaction: any) {
            await interaction.deferReply();
            await db.del("main", "timetableData", {
                grade: interaction.options.getString("学年"),
                department: interaction.options.getString("学科"),
                day: String(interaction.options.getInteger("削除日")),
            });
            await db.del("main", "timetableData", {
                grade: interaction.options.getString("学年"),
                department: interaction.options.getString("学科"),
    ...
    },
    {
        ...
        async execute(interaction: any) {
            ...
            const modal = new ModalBuilder()
                ...
                .setTitle(
                    `${departmentData[parseFloat(interaction.options.getString("学科")) - 1].name}${interaction.options.getString(
                        "学年",
                    )}年${Math.floor(date / 100)}月${Math.floor(date % 100)}日`,
                );
