...
export default [
    ...
    },
    {
        ...
        async execute(interaction: any) {
            ...
            await db.updateOrInsert("main", "timetableData", {
                grade: interaction.options.getString('学年'),
                department: interaction.options.getString('学科'),
                day: String(interaction.options.getInteger('変更日') + '00')
            }, defaultData[0]);
            ...
            if (interaction.options.getString('モード') === '1') {
                await interaction.editReply({
                    embeds: [embed],
                    components: [{type: 1, components: [select[0]]}, {type: 1, components: [select[1]]}, {
                        type: 1,
                        components: [select[2]]
                    }, {type: 1, components: [select[3]]}, {type: 1, components: [button]}]
                });
            } else {
                await interaction.editReply({
                    embeds: [embed],
    ...
    },
    {
        ...
        async execute(interaction: any) {
            ...
            if (data.length !== 0) {
                ...
                const replyOptions = (time: any) => {
                    return {content: '指定した学科・学年の時間割データが見つかりませんでした。\n(このメッセージは' + time + '秒後に自動で削除されます)', ephemeral: true};
                };
