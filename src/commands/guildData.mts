...
export default [
    {
        data: new SlashCommandBuilder()
            ...
            .addRoleOption((option: any) =>
                option.setName("m科ロール").setDescription("m科用ロールを指定してください。").setRequired(false),
            )
            .addChannelOption((option: any) =>
                option.setName("e科チャンネル").setDescription("e科用チャンネルを指定してください。").setRequired(false),
            )
            .addRoleOption((option: any) =>
                option.setName("e科ロール").setDescription("e科用ロールを指定してください。").setRequired(false),
            )
            .addChannelOption((option: any) =>
                option.setName("d科チャンネル").setDescription("d科用チャンネルを指定してください。").setRequired(false),
            )
            .addRoleOption((option: any) =>
                option.setName("d科ロール").setDescription("d科用ロールを指定してください。").setRequired(false),
            )
            .addChannelOption((option: any) =>
                option.setName("j科チャンネル").setDescription("j科用チャンネルを指定してください。").setRequired(false),
            )
            .addRoleOption((option: any) =>
                option.setName("j科ロール").setDescription("j科用ロールを指定してください。").setRequired(false),
            )
            .addChannelOption((option: any) =>
                option.setName("c科チャンネル").setDescription("c科用チャンネルを指定してください。").setRequired(false),
            )
            .addRoleOption((option: any) =>
                option.setName("c科ロール").setDescription("c科用ロールを指定してください。").setRequired(false),
            ),
    ...
    },
    {
        ...
        async execute(interaction: any) {
            ...
            while (flag === -1) {
                await reply
                    .awaitReactions({
                        filter: (reaction: any) => reaction.emoji.name === "⭕" || reaction.emoji.name === "❌",
                        max: 1,
                        time: 60_000,
                    })
                    .then(() => {
