    ...
    if (interaction.customId === "selectCat") {
        ...
        //カテゴリ受付
            ...
            ccconfig.guilds
                ...
                ccconfig.guilds
                    .find((guild) => guild.ID === interaction.guild.id)
                    .categories.find((category) => category.ID === interaction.values[0]).channels.length
            ] = {
                ID: newChannel.id,
            ...
            if (
                ccconfig.guilds
                    .find((guild) => guild.ID === interaction.guild.id)
                    .categories.find((category) => category.ID === interaction.values[0]).allowRole
            ) {
                const mkRole = new ActionRowBuilder().addComponents(
        ...
        if (interaction.values[0].split("/")[2] === "1") {
            ...
            const newRole = await interaction.guild.roles.create({
                name: ccconfig.guilds
                    ...
                    .channels.find((channel) => channel.ID === interaction.values[0].split("/")[1]).name,
            ...
            ccconfig.guilds
                .find((guild) => guild.ID === interaction.guild.id)
                .categories.find((category) => category.ID === interaction.values[0].split("/")[0])
                .channels.find((channel) => channel.ID === interaction.values[0].split("/")[1]).thereRole = true;
            ccconfig.guilds
                .find((guild) => guild.ID === interaction.guild.id)
                .categories.find((category) => category.ID === interaction.values[0].split("/")[0])
                .channels.find((channel) => channel.ID === interaction.values[0].split("/")[1]).roleID = newData.roleID;
            ccconfig.guilds
                .find((guild) => guild.ID === interaction.guild.id)
                .categories.find((category) => category.ID === interaction.values[0].split("/")[0])
...
cron.schedule("0 20 * * 0,1,2,3,4", async () => {
    ...
    if (timetable === true) {
        ...
        await (client.channels.cache.get(config.E) ?? (await client.channels.fetch(config.E))).send({
            embeds: [timetableBuilder(Classes.E, dayOfWeek)],
        });
        await (client.channels.cache.get(config.D) ?? (await client.channels.fetch(config.D))).send({
            embeds: [timetableBuilder(Classes.D, dayOfWeek)],
        });
        await (client.channels.cache.get(config.J) ?? (await client.channels.fetch(config.J))).send({
            embeds: [timetableBuilder(Classes.J, dayOfWeek)],
        });
        await (client.channels.cache.get(config.C) ?? (await client.channels.fetch(config.C))).send({
            embeds: [timetableBuilder(Classes.C, dayOfWeek)],
        });
    }
});
